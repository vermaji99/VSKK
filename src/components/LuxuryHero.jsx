...
const LuxuryHero = ({ onHeroComplete }) => {
  ...
  useEffect(() => {
    ...
    if (scrollProgress >= 1 && !isHeroComplete) {
  ...
  }, [scrollProgress, renderCanvas, isHeroComplete, onHeroComplete]);
  ...
  // Scroll tracking for cinematic experience
  useEffect(() => {
    ...
    const updateScroll = () => {
      ...
      const heroHeight = containerRef.current?.offsetHeight || innerHeight * 3;
      const rawProgress = Math.min(Math.max(scrollY / (heroHeight - innerHeight), 0), 1);

      // Smooth the progress slightly
      const newProgress = lastProgress + (rawProgress - lastProgress) * 0.3;
  ...
  }, [isReady, isHeroComplete, scrollProgress]);
  ...
  return (
    <section
      ...
      style={{
        ...
        height: `${100 * 3}vh` // 3x viewport height for cinematic scroll
      }}
    >
      {/* Loading indicator */}
      {!isReady && (
